...
class Critic3D(nn.Module):
    ...
    def forward(self, x):
        ...
        out_spatial = F.adaptive_avg_pool3d(out_spatial, 1).view(B, -1)  # [B,128]

        out = out_spatial

        # ---- FFT Arm ----
        if self.fft_arm:
            x_fft = torch.fft.fft(x, dim=1)       # [B,C,H,W] complex
            x_fft = torch.view_as_real(x_fft)     # [B,C,H,W,2]
            x_fft = x_fft.permute(0,4,1,2,3)      # [B,2,C,H,W]
            out_fft = self.fft_conv(x_fft)
            out_fft = F.adaptive_avg_pool3d(out_fft, 1).view(B, -1)  # [B,128]

          
            out = torch.cat([out_spatial, out_fft], dim=1)  # [B,256]
